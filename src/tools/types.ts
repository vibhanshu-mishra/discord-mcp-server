 ...
 * this tool mutate Discord itself? It is the gate read-only mode enforces.
 * `defineTool` records it for every registered definition: omitted metadata
 * follows `readOnlyHint` with a fail-closed default. An analytics tool that writes
 * only the local database is `readOnlyHint: false` (it has a side effect) but
 * `discordWrite: false` (it never touches Discord). See `mutatesDiscord` in
 * `../readonly.ts`.
 */
